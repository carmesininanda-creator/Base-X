...
def _system_prompt(user_id, user_message=""):
    ...
    if any("[de partida]" in f["content"] for f in facts):
        facts_str += ("\n- ATENÇÃO: itens \"[de partida]\" foram semeados no cadastro pela "
                      ...
                      "convites de conexão, nunca rótulos. SONHOS são hipóteses VIVAS, "
                      "jamais fatos imutáveis: podem mudar, evoluir, desaparecer ou ser "
                      "substituídos — sua função é acompanhar a EVOLUÇÃO da pessoa, nunca "
                      "preservar o sonho; e você NUNCA empurra ninguém para realizar um "
                      "sonho — ajuda quando A PESSOA mostrar que ele continua vivo "
                      "(sonho que mudou: atualize com naturalidade e celebre a evolução). "
                      "Sobre TERCEIROS dentro destes "
